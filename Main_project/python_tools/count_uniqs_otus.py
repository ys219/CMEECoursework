...
if __name__ == "__main__":
    # parser settings
    #
    parser = argparse.ArgumentParser(description = "This tool count the OTUs in each sample
    ...
    # otu before input
    parser.add_argument("-b","--unfil_otu_map", help = "uniqs vs OTUs for unfiltered(but quality filtered and dereplcated) reads; geerated with vsearch in blast6out method", metavar = "unfil_otu.txt")
    ...
    # otu after input
    parser.add_argument("-a","--fil_otu_map", help = "uniqs vs. OTUs for filtered reads", metavar = "fil_otu.txt")#
    parser.add_argument("-m","--otus_b_a_map", help = "unfil_otu vs fil_otu map with blast6out method", metavar = "OTUs_map.txt")#
    ...
    # bef_map = open(args.unfil_otu_map,"r")
    bef_dic, u_bef_count = extract_dictionary(args.unfil_otu_map)
    ...
    # aft_map = open(args.fil_otu_map,"r")
    aft_dic, u_aft_count = extract_dictionary(args.fil_otu_map)
    ...
    print("%d uniqs got both unfil_otu&aft"%(len(set(bef_dic.keys())&set(aft_dic.keys()))))#7
    ...
    #replace uniqs in sample_uniq_dic with unfil_otu
    sample_befOTU_dic = {}
    ...
    #
    #replace uniqs in sample_uniq_dic with fil_otu
    sample_aftOTU_dic = {}
    for k in sample_uniqs_dic:
